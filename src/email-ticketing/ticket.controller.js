...
exports.replyToTicket = asyncHandler(async (req, res) => {
  ...
  const ticket = await EmailTicket.findOne({ ticketId, organization }).populate('assignedTo', 'role');
  ...
  // ==================== TIER-BASED REPLY BLOCKING FOR DEV-ESCALATED TICKETS ====================
  // Only Dev role and Admins can reply to dev-escalated tickets
  // Block: Agent (Tier-1), QA (Tier-2), TL (Tier-2)
  const isEscalatedToDev = ticket.escalatedToDev || (ticket.assignedTo && ticket.assignedTo.role === 'Dev');
  ...
  if (isEscalatedToDev && !isAdminRole && !isDevRole) {
    return res.status(403).json({
      success: false,
      message: 'This ticket has been escalated to Dev team. Only Tier-3 Dev agents can reply to it.',
    });
  }
...
exports.listTickets = asyncHandler(async (req, res) => {
  ...
  // ==================== TIER-BASED FILTERING FOR DEV-ESCALATED TICKETS ====================
  // Only Dev role and Admins can see dev-escalated tickets
  // Block: Agent (Tier-1), QA (Tier-2), TL (Tier-2)
  const userRole = req.user?.role;
  const isAdminRole = ['Admin', 'SuperAdmin'].includes(userRole);
  const isDevRole = userRole === 'Dev';

  // If user is NOT Dev and NOT Admin, exclude dev-escalated tickets from list
  if (!isAdminRole && !isDevRole) {
    mongoFilter = {
      ...mongoFilter,
      $and: [
        mongoFilter.$and ? { $and: mongoFilter.$and } : {},
        {
          $or: [
            { escalatedToDev: { $ne: true } }, // Not escalated to dev
            { escalatedToDev: { $exists: false } }, // Field doesn't exist (old tickets)
          ],
        },
      ],
    };
...
exports.getTicket = asyncHandler(async (req, res) => {
  ...
  // Tier-based read blocking: lower tiers cannot read higher-tier escalations.
  const ticket = data.ticket || data;
  const userRole = req.user?.role;
  const userTier = req.user?.tier;
  const isAdminRole = ['Admin', 'SuperAdmin'].includes(userRole);

  // Check if escalated to Dev (Tier-3)
  const isEscalatedToDev = ticket.escalatedToDev || (ticket.assignedTo && ticket.assignedTo.role === 'Dev');

  // STRICT BLOCKING: Dev-escalated tickets (Tier-3) are only visible to Tier-3 Dev and Admins
  // Block ALL non-dev roles: Agent (Tier-1), QA (Tier-2), TL (Tier-2)
  const isDevRole = userRole === 'Dev';
      ...
      messages: [
        {
          ...
          createdAt: ticket.escalatedAt || new Date(),
        },
      ],
      _tierBlocked: true,
      _devEscalated: true,
  ...
  // General tier-based blocking for non-dev escalations
  const tierOrder = { 'Tier-1': 1, 'Tier-2': 2, 'Tier-3': 3 };
  const userTierNum = tierOrder[userTier] || 0;
  const escalatedTierNum = tierOrder[ticket.escalatedToTier] || 0;

  if (!isAdminRole && userTierNum > 0 && escalatedTierNum > userTierNum && !isEscalatedToDev) {
    return res.json({
      success: true,
      ...data,
      _tierBlocked: true,
      _tierBlockMessage: `This ticket has been escalated to ${ticket.escalatedToTier}. Only that tier and above can read it.`,
    });
  }
  ...
  res.json({
    success: true,
    ...data,
    escalatedToDev: isEscalatedToDev
  });
});
...
exports.assignTicket = asyncHandler(async (req, res) => {
  ...
  let ticket = await EmailTicket.findOne({ ticketId: id, organization }).populate('assignedTo', 'role');
  if (!ticket) return res.status(404).json({ success: false, message: 'Ticket not found' });

  // ==================== TIER-BASED ASSIGNMENT BLOCKING FOR DEV-ESCALATED TICKETS ====================
  // Only Dev role and Admins can re-assign dev-escalated tickets
  // Block: Agent (Tier-1), QA (Tier-2), TL (Tier-2)
  const isEscalatedToDev = ticket.escalatedToDev || (ticket.assignedTo && ticket.assignedTo.role === 'Dev');
  const userRole = req.user?.role;
  const isAdminRole = ['Admin', 'SuperAdmin'].includes(userRole);
  ...
  if (isEscalatedToDev && !isAdminRole && !isDevRole) {
    return res.status(403).json({
      success: false,
      message: 'This ticket has been escalated to Dev team. Only Tier-3 Dev agents can re-assign it.',
    });
  }
  ...
  // Only the current owner, or a Manager/Admin/QA can re-assign (escalate) it.
  // Check role if not current owner
  const isOwner = ticket.assignedTo && ticket.assignedTo.toString() === currentUserId.toString();
  const isManager = ['admin', 'superadmin', 'manager', 'qa', 'tl'].includes((req.user?.role || '').toLowerCase());
  
  if (ticket.assignedTo && !isOwner && !isManager) {
    return res.status(403).json({
      success: false,
      message:
        'This ticket is already assigned to another user. Only the current owner or a manager can re-assign or escalate it.',
    });
  }
  ...
  if (assignedToId && assignedToId.toString() !== (previousAgent ? previousAgent.toString() : '')) {
     ...
     // We treat direct assignment as 'Accepted' immediately.
     
     const historyEntry = {
       fromAgent: previousAgent || null,
       fromAgentName: fromAgentUser ? (fromAgentUser.alias || fromAgentUser.name) : 'Unassigned',
       toAgent: assignedToId,
       toAgentName: toAgentUser ? (toAgentUser.alias || toAgentUser.name) : 'Unknown',
       transferredAt: new Date(),
       reason: reason || (previousAgent ? 'Escalation/Reassignment' : 'Initial Assignment'),
       notes: notes || '', // Save optional notes
       status: 'Accepted', // Direct assignment
       requestedAt: new Date(),
       acceptedAt: new Date()
     };
     
     // We need to push this. updateData matches simple fields. 
     // For array push we might need a separate operation or include it in update if using save() pattern, 
     // but here we use findOneAndUpdate.
     // We'll add $push to the update operation.
     updateData.$push = { transferHistory: historyEntry };
  }
  ...
  ticket = await EmailTicket.findOneAndUpdate({ ticketId: id, organization }, updateData, {
    ...
    .populate('transferHistory.fromAgent', 'name alias')
    .populate('transferHistory.toAgent', 'name alias');

  if (!ticket) return res.status(404).json({ success: false, message: 'Ticket not found' });

  // Add system message about assignment/escalation
  const TicketMessage = require('./models/TicketMessage');
  let systemMsg = '';
  if (assignedToId) {
      const assigneeName = ticket.assignedTo.alias || ticket.assignedTo.name;
      if (previousAgent) {
           systemMsg = `Ticket escalated/reassigned to ${assigneeName}. Reason: ${reason || 'None'}`;
      } else {
           systemMsg = `Ticket assigned to ${assigneeName}`;
      }
      
      if (notes) {
          systemMsg += `. Notes: ${notes}`;
      }
  } else {
      systemMsg = `Ticket unassigned`;
  }
...
async function validateApiKey(req, res, next) {
  ...
  // For ShyEyes integration, use a hardcoded organization ID that matches your setup
  // This should match the organization ID in your OrgEmailConfig
  req.organizationId = '690afe4c071944b493b73862'; // The org ID from your IMAP setup

  console.log(`[VALIDATE_API_KEY] Set organization ID: ${req.organizationId}`);

  // TODO: Add proper API key validation against database in production
  next();
}
      ...
      try {
        ...
        if (emailConfig && emailConfig.emailAddress) {
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px; background: #ffffff; border: 1px solid #e5e5e5;">
            <h2 style="color: #333; margin-bottom: 20px; border-bottom: 2px solid #10B981; padding-bottom: 10px;">New Support Ticket from Website</h2>
      ...
      try {
        if (emailConfig && emailConfig.emailAddress) {
          ...
          await sendEmailReply({
            ...
            html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px; background: #ffffff; border: 1px solid #e5e5e5;">
            <h2 style="color: #333; margin-bottom: 20px; border-bottom: 2px solid #4F46E5; padding-bottom: 10px;">Support Ticket Created</h2>
...
exports.getTicketMessagesFromWidget = [
  ...
  asyncHandler(async (req, res) => {
    ...
    const ticket = await EmailTicket.findOne({ ticketId, customerEmail }).populate(
      'assignedTo',
      'role'
    );
    if (!ticket) {
    ...
    // If escalated to Dev (Tier-3), hide chat and show escalation message
    // This applies when ticket is escalated by Tier-2, TL, or QA to Dev role
    if (ticket.escalatedToDev || (ticket.assignedTo && ticket.assignedTo.role === 'Dev')) {
      return res.json({
        success: true,
        escalatedToDev: true,
        messages: [
...
// ==================== ESCALATION / TRANSFER FUNCTIONALITY ====================

// Helper: Build escalation chain from transferHistory
function buildEscalationChain(ticket) {
  if (!ticket || !Array.isArray(ticket.transferHistory)) return [];
  return ticket.transferHistory.map((t, idx) => ({
    step: idx + 1,
    from: {
    ...
    notes: t.notes || null, // Include notes in helper
    requestedAt: t.requestedAt || t.transferredAt,
    acceptedAt: t.acceptedAt || null,
  }));
}

// Transfer/Escalate Ticket - POST /:ticketId/transfer
exports.transferTicket = asyncHandler(async (req, res) => {
  const organization = getOrgId(req);
  const currentAgentId = req.user?.id || req.user?._id;
  const { ticketId } = req.params;
  ...
  // Escalation policy:
  // Agents (Tier-1): Only to QA/TL. Not to other Agents or Dev.
  // QA/TL (Tier-2): To Agents (de-escalate) or Dev (escalate).
  // Dev (Tier-3): Cannot initiate.
  ...
  if (!allowedRecipients.includes(toAgent.role)) {
    return res.status(400).json({
      success: false,
      message: 'Ticket can only be transferred to an Agent, QA, TL, or Dev',
    });
  }
  // Agents cannot transfer to other Agents or directly to Dev
  if (currentAgent.role === 'Agent' && toAgent.role === 'Agent') {
    return res.status(403).json({
      success: false,
      message: 'Agents cannot transfer to other Agents. Escalate to QA or TL instead.',
    });
  }
  if (currentAgent.role === 'Agent' && toAgent.role === 'Dev') {
    return res.status(403).json({
      success: false,
      message: 'Agents cannot escalate directly to Dev. Escalate to QA or TL first.',
    });
  }
  // Only QA/TL can escalate to Dev
  const tierOrder2 = { 'Tier-1': 1, 'Tier-2': 2, 'Tier-3': 3 };
  const toTierNum2 = tierOrder2[toAgent.tier] || 1;
  if (toTierNum2 === 3 && !['QA', 'TL'].includes(currentAgent.role)) {
    return res.status(403).json({
      success: false,
  ...
  // Only allow transfer if current user is assigned to the ticket
  if (!ticket.assignedTo) {
    return res.status(400).json({
      success: false,
      message: 'Please accept this transferred ticket first before transferring it again',
    });
  }

  if (ticket.assignedTo.toString() !== currentAgentId.toString()) {
    return res.status(403).json({
      success: false,
  ...
  ticket.transferHistory.push(transferEntry);

  // ==================== TIER-BASED ESCALATION TRACKING ====================
  const tierOrder = { 'Tier-1': 1, 'Tier-2': 2, 'Tier-3': 3 };
  const fromTierNum = tierOrder[currentAgent.tier] || 0;
  const toTierNum = tierOrder[toAgent.tier] || 0;

  if (toTierNum > fromTierNum && toTierNum >= 2) {
    ticket.escalatedToTier = toAgent.tier;
    ticket.escalatedAt = new Date();
    ticket.escalatedFromTier = currentAgent.tier || 'Tier-1';
    // Only set escalatedToDev when escalating specifically to Dev (Tier-3)
    if (toAgent.role === 'Dev' || toAgent.tier === 'Tier-3') {
      ticket.escalatedToDev = true;
    }
  }

  // Mark ticket as Transferred and clear assignment until recipient accepts
  ...
  // Add system message to ticket messages
  const TicketMessage = require('./models/TicketMessage');
  const isDevEscalation = ticket.escalatedToDev;
  await TicketMessage.create({
    ticketId: ticket.ticketId,
    organization,
    senderType: 'system',
    ...
    message: isDevEscalation
      ...
      : `Ticket escalated from ${currentAgent.alias || currentAgent.name} to ${toAgent.alias || toAgent.name}`,
    html: isDevEscalation
      ? `<p>Escalated to Dev - Ticket will be resolved by the Dev team within 24 to 48 hours.</p>`
      : `<p>Ticket escalated from ${currentAgent.alias || currentAgent.name} to ${toAgent.alias || toAgent.name}</p>`,
  });

  // Update sender status if no other active tickets
  const activeTicketsCount = await EmailTicket.countDocuments({
    assignedTo: currentAgentId,
    status: { $in: ['open'] },
    organization,
  });

  if (activeTicketsCount === 0 && currentAgent.workStatus === 'busy') {
  ...
  if (ticketNamespace) {
    ...
    if (ticketNamespace.to) {
      ticketNamespace.to(targetRoom).emit('ticket-transfer-requested', {
        ticketId,
        from: {
          id: currentAgent._id,
    ...
    // Broadcast update
    if (ticketNamespace.emitTicketUpdate) {
      ticketNamespace.emitTicketUpdate(ticket);
    }
  }
...
// Accept Transfer Request - POST /:ticketId/transfer/accept
exports.acceptTransferRequest = asyncHandler(async (req, res) => {
  const organization = getOrgId(req);
  ...
  if (ticket.status !== 'transferred') {
    return res.status(400).json({
      success: false,
      message: 'Ticket is not in transferred status',
    });
  }

  // Find latest transfer request
  const latestTransfer = ticket.transferHistory?.length
    ? ticket.transferHistory[ticket.transferHistory.length - 1]
    : null;

  if (!latestTransfer || latestTransfer.status !== 'Requested') {
    return res.status(400).json({
      success: false,
      message: 'No pending transfer request found',
    });
  }

  // Ensure this agent is the intended recipient
  if (latestTransfer.toAgent.toString() !== agentId.toString()) {
    return res.status(403).json({
      success: false,
  ...
  // Assign ticket to accepting agent
  ticket.status = 'open';
  ticket.assignedTo = agentId;
  ticket.assignedBy = null; // Clear assignedBy since agent accepted themselves
  ...
  agent.lastStatusChangeTime = new Date();
  ...
  await TicketMessage.create({
    ...
    senderType: 'system',
    senderName: 'System',
    message: `Transfer accepted by ${agent.alias || agent.name}`,
    html: `<p>Transfer accepted by ${agent.alias || agent.name}</p>`,
  });

  // Populate ticket before sending response
  await ticket.populate('assignedTo', 'name email alias role');

  // Emit socket events
  const ticketNamespace = req.app.get('ticketNamespace');
  if (ticketNamespace) {
    if (ticketNamespace.emitTicketAssigned && ticket.assignedTo) {
      ticketNamespace.emitTicketAssigned(ticket, ticket.assignedTo);
    }
    if (ticketNamespace.emitTicketUpdate) {
      ticketNamespace.emitTicketUpdate(ticket);
...
// Get Escalation Chain - GET /:ticketId/escalation-chain
exports.getEscalationChain = asyncHandler(async (req, res) => {
  const organization = getOrgId(req);
  const { ticketId } = req.params;
  const userId = req.user?.id || req.user?._id;
  const userRole = req.user?.role;
  ...
  const ticket = await EmailTicket.findOne({ ticketId, organization })
    .populate('transferHistory.fromAgent', 'name role alias')
    .populate('transferHistory.toAgent', 'name role alias');

  if (!ticket) {
    return res.status(404).json({ success: false, message: 'Ticket not found' });
  }

  // Authorization: QA, TL, Admin can view any; assigned agent can view
  const isAssignedAgent = ticket.assignedTo && ticket.assignedTo.toString() === userId.toString();
  ...
  const chain = buildEscalationChain(ticket);
  ...
  res.json({
    ...
    data: {
      ...
      activeAssignee: ticket.assignedTo
        ? {
            id: ticket.assignedTo,
            name: ticket.assignedTo.name || ticket.assignedTo.alias,
...
// Add Remark - POST /tickets/:id/remark
exports.addRemark = asyncHandler(async (req, res) => {
  const organization = getOrgId(req);
  const { id } = req.params;
  const { remark } = req.body;
  const userId = req.user?.id || req.user?._id;
  const userName = req.user?.alias || req.user?.name;
  const userRole = req.user?.role;
  ...
  // Authorization: Only QA, TL, Admin, or assigned agent can add remarks
  const isAssignedAgent = ticket.assignedTo && ticket.assignedTo.toString() === userId.toString();
  const privilegedRoles = ['QA', 'TL', 'Admin', 'SuperAdmin'];
  ...
  if (!isAssignedAgent && !privilegedRoles.includes(userRole)) {
    return res.status(403).json({
      success: false,
      message: 'Not authorized to add remarks to this ticket',
    });
  }

  // Add remark as a system message
  await EmailTicketMessage.create({
    ticketId: ticket.ticketId,
    organization,
    senderType: 'system',
    senderName: 'System',
    message: `Remark by ${userName} (${userRole}): ${remark}`,
    html: `<p><strong>Remark by ${userName} (${userRole}):</strong> ${remark}</p>`,
  });

  // Update ticket's last activity time
  ticket.lastActivityAt = new Date();
  await ticket.save();
