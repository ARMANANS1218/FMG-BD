  ...
  queryNamespace.use(async (socket, next) => {
    try {
      ...
      if (user) {
        ...
        const customer = await Customer.findById(socket.userId).select('-password');
        if (customer) {
          socket.userName = customer.name;
          socket.userRole = 'Customer';
          socket.organizationId = customer.organizationId;
          socket.isCustomer = true;
          console.log(
            `✅ Query namespace auth (Customer): ${customer.name} - Org: ${customer.organizationId}`
          );
        }
      }
  ...
  queryNamespace.on('connection', (socket) => {
    ...
    socket.on('send-query-message', async ({ petitionId, userId, message }) => {
      try {
        ...
        // ==================== DEV ESCALATION BLOCKING FOR MESSAGES ====================
        // If query is escalated to Dev, only Tier-3 Dev role can send messages.
    ...
    socket.on('transfer-query', async ({ petitionId, fromAgentId, toAgentId, reason }) => {
      try {
        ...
        if (fromAgent.role === 'Agent' && toAgent.role === 'Agent') {
          socket.emit('error', {
            message: 'Agents cannot transfer to other Agents. Escalate to QA or TL instead.',
          });
          return;
        }
        if (fromAgent.role === 'Agent' && toAgent.role === 'Dev') {
          socket.emit('error', {
        ...
        if (toTierNum === 3 && !['QA', 'TL'].includes(fromAgent.role)) {
          socket.emit('error', {
        ...
        if (['QA', 'TL'].includes(fromAgent.role) && ['QA', 'TL'].includes(toAgent.role)) {
          socket.emit('error', {
        ...
        // Dev-escalated queries can only be transferred by Tier-3 Dev.
        if (query.escalatedToDev && fromAgent.role !== 'Dev') {
          socket.emit('error', {
