...
const {
  ...
  isAdmin,
  isAdminOrTL,
  isAdminTLOrManagement,
} = require('../utils/validateToken');
const {
  getLoginTimings,
  updateLoginTimings,
  getLoginTimingStatus,
  toggleLoginTiming,
} = require('../controllers/loginTiming.controller');
...
router.get('/activity/report', validateToken, isAdminTLOrManagement, getActivityReport);
...
// Get all employees (Admin, Agent, QA, TL) - Admin, TL, and Management can access
router.get('/employees', validateToken, isAdminTLOrManagement, getAllEmployees);
...
// ==================== LOGIN TIMINGS MANAGEMENT ====================
// Get login timings for organization (Admin only)
router.get('/login-timings', validateToken, isAdmin, getLoginTimings);
// Update login timings (Admin only)
router.put('/login-timings', validateToken, isAdmin, updateLoginTimings);
// Toggle login timing active status (Admin only)
router.put('/login-timings/toggle', validateToken, isAdmin, toggleLoginTiming);
// Get login timing status (public - for login page)
router.get('/login-timings/status', getLoginTimingStatus);
