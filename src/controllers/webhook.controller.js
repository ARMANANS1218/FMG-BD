...
exports.createGuestConversation = async (req, res) => {
  try {
    ...
    console.log('📥 Guest conversation request:', {
    ...
    // Normalize priority & department (category)
    ...
    console.log(
      `✅ Guest conversation created: ${conversationId} | Petition: ${petitionId} | Org: ${organization.name}`
    );
      ...
      if (io) {
        ...
        for (const socket of roomSockets) {
          ...
          if (targetUserIds.includes(socketUserId)) {
            ...
            if (isTrulyBusy) {
              console.log(`🚫 [Guest] Skipped busy agent (Double Check): ${socketUserId}`);
              continue;
            }
        ...
        console.log(
          `📊 [Guest] Notification Summary: Sent to ${sentCount}/${roomSockets.length} sockets in ${orgRoom}`
        );
      } else {
...
exports.sendWidgetMessage = async (req, res) => {
  try {
    ...
    if (!message || (!conversationId && !petitionId)) {
      return res.status(400).json({
        success: false,
        message: 'message and (conversationId or petitionId) are required',
      });
    }
    ...
    const query = await Query.findOne(
      petitionId
        ? { petitionId, organizationId: organization._id }
        : { conversationId, organizationId: organization._id }
    );
    if (!query) {
...
exports.uploadWidgetSnapshot = async (req, res) => {
  try {
    ...
    if (!query) {
      ...
      return res
        .status(404)
        .json({ success: false, message: 'Petition not found for organization' });
    }
...
/**
 * Submit chat rating from Chat Widget
 */
exports.submitWidgetRating = async (req, res) => {
  try {
    const { apiKey, petitionId, rating, comment } = req.body;
    ...
    if (!organization) {
      return res
        .status(401)
        .json({ success: false, message: 'Invalid API key or organization not found' });
    }
    ...
    console.log(`✅ Rating submitted for petition ${petitionId}: ${rating} stars`);
    ...
    console.error('❌ submitWidgetRating error:', error);
    return res.status(500).json({ success: false, message: 'Failed to submit feedback' });
  }
};

/**
 * Check if rating already exists for a petition (widget API key auth)
 */
exports.checkWidgetRating = async (req, res) => {
  try {
    const { apiKey, petitionId } = req.query;

    if (!apiKey || !petitionId) {
      return res
        .status(400)
        .json({ success: false, message: 'apiKey and petitionId are required' });
    }
    ...
    const hasRating = !!(query.feedback && query.feedback.rating);

    return res.status(200).json({
      success: true,
      hasRating,
      feedback: hasRating ? query.feedback : null,
    });
  } catch (error) {
    console.error('❌ checkWidgetRating error:', error);
    return res.status(500).json({ success: false, message: 'Failed to check rating' });
  }
};
