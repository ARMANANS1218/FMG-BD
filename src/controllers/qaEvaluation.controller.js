...
function ensureQAOrTL(req, res) {
  const allowed = ['QA', 'TL', 'Management'];
...
exports.createEvaluation = async (req, res) => {
  ...
  try {
    ...
    if (!query.assignedTo)
      return res.status(400).json({ status: false, message: 'Query has no assigned agent' });
    ...
    if (existing) {
      ...
      Object.assign(existing, {
        ...
        ...Object.entries(scores || {}).reduce((acc, [k, v]) => {
          acc[k] = { score: Number(v) || 0 };
          return acc;
        }, {}),
      });
      await existing.save();
...
exports.getByPetition = async (req, res) => {
  try {
    ...
    const evalDoc = await QueryEvaluation.findOne({ petitionId }).populate(
      'evaluatedBy',
      'name email'
    );
    if (!evalDoc)
      return res.status(404).json({ status: false, message: 'Not rated yet', data: null });
    ...
    const pipeline = [
      Object.keys(match).length ? { $match: match } : null,
      {
        $group: {
          _id: '$agentId',
          count: { $sum: 1 },
          avgScore: { $avg: '$totalWeightedScore' },
        ...
        $project: {
          ...
          passRate: {
            $cond: [
              { $eq: ['$count', 0] },
              0,
              { $round: [{ $multiply: [{ $divide: ['$passCount', '$count'] }, 100] }, 2] },
            ],
          },
        },
      },
