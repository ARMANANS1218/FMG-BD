...
exports.createShift = async (req, res) => {
  try {
    const { shiftName, startTime, endTime, duration, checkInGracePeriod, checkOutGracePeriod } = req.body;
    ...
    const shift = new Shift({
      ...
      endTime,
      duration,
      checkInGracePeriod: checkInGracePeriod ?? 10,
      checkOutGracePeriod: checkOutGracePeriod ?? 10,
      createdBy: userId
    });
...
exports.updateShift = async (req, res) => {
  try {
    const { id } = req.params;
    const { shiftName, startTime, endTime, duration, isActive, checkInGracePeriod, checkOutGracePeriod } = req.body;
    const { organizationId, id: userId, role } = req.user;
    ...
    if (typeof checkInGracePeriod !== 'undefined') shift.checkInGracePeriod = checkInGracePeriod;
    if (typeof checkOutGracePeriod !== 'undefined') shift.checkOutGracePeriod = checkOutGracePeriod;
