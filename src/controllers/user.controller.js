...
exports.register = async (req, res) => {
  try {
    const { user_name, name, department, tier, mobile, email, password, role, employee_id } =
      req.body;
    const customRole = req.body.customRole || null;
    ...
    const adminAccessCustomRoles = ['client', 'aggregator'];
    const normalizedCustomRole = (customRole || '').trim().toLowerCase();

    // ✅ Prevent Admin from creating another Admin role
    // Exception: allow explicit management login presets that must have Admin access.
    if (
      creatorRole === 'Admin' &&
      role === 'Admin' &&
      !adminAccessCustomRoles.includes(normalizedCustomRole)
    ) {
      return res.status(403).json({
        message:
          'Admin users can create Agent, QA, TL, Dev, and Management roles. Admin role is only allowed for Client or Aggregator custom roles.',
        status: false,
      });
    }

    const isAdminAccessCustomRole = adminAccessCustomRoles.includes(normalizedCustomRole);
    ...
    const rolesRequiringTier = ['Agent', 'QA', 'TL', 'Dev'];
    if (rolesRequiringTier.includes(role)) {
      if (!department) {
        return res.status(400).json({
          message: 'Department is required for Agent, QA, TL and Dev roles',
          status: false,
        });
      }
      if (!tier) {
    ...
    const newAuthor = await User.create({
      ...
      customRole, // Dynamic role name (optional)
...
exports.loginUser = async (req, res) => {
  try {
    const noPasswordAttemptBlockRoles = ['Admin', 'Management'];
    ...
    // 🔹 Both employee_id AND email are required and must match the same user
    if (!empId || !cleanEmail) {
    ...
    if (user.blockedReason) {
      // Allow Management users to login even if they were previously blocked
      // due to old failed-password-attempt policy.
      if (
        noPasswordAttemptBlockRoles.includes(user.role) &&
      ...
      let blockMessage = 'Account blocked. Please contact Administrator.';

      if (user.blockedReason === 'Account deactivated by Admin') {
        blockMessage = 'Your ID is deactivated.';
      } else if (user.blockedReason === 'Multiple failed login attempts') {
        blockMessage =
          'Your ID is blocked due to multiple invalid attempts. Please contact Administrator.';
      } else if (user.blockedReason) {
        blockMessage = `${user.blockedReason}. Please contact Administrator.`;
      }
    ...
    if (!passwordMatch) {
      ...
      if (!noPasswordAttemptBlockRoles.includes(user.role)) {
        ...
        // Admin/Management - just return wrong password without counting attempts
        console.log(`❌ Wrong password for ${user.role} - no blocking`);
        return res.status(401).json({
          status: false,
    ...
    if (!noPasswordAttemptBlockRoles.includes(user.role) && user.failedLoginAttempts > 0) {
      user.failedLoginAttempts = 0;
    ...
    // ==================== LOGIN TIMING ENFORCEMENT ====================
    try {
      const LoginTiming = require('../models/LoginTiming');
      const loginTiming = await LoginTiming.findOne({ organizationId: user.organizationId });
      ...
      if (loginTiming && loginTiming.isActive) {
        const normalizedCustomRole = normalizeCustomRole(user.customRole || '');
        const mappedCustomRole = customRoleMap[normalizedCustomRole] || null;
        const roleCandidates = [user.role, mappedCustomRole].filter(Boolean);
        const hasManagementUmbrella =
          loginTiming.restrictedRoles.includes('Management') && user.role === 'Management';

        const isRestrictedRole =
          roleCandidates.some((roleKey) => loginTiming.restrictedRoles.includes(roleKey)) ||
          hasManagementUmbrella;

        console.log('🕐 Login Timing Check:', {
          userId: user._id,
          userRole: user.role,
          customRole: user.customRole,
          mappedCustomRole,
          restrictedRoles: loginTiming.restrictedRoles,
          timingWindow: `${loginTiming.startTime} - ${loginTiming.endTime}`,
        });

        // Check if role is affected by login timing
        if (isRestrictedRole) {
          const isAllowed = isWithinWindow(loginTiming.startTime, loginTiming.endTime);

          if (!isAllowed) {
            const nextStartTime = loginTiming.getNextLoginStartTime();
            console.log(
              `🚫 [LOGIN BLOCKED] Login window closed for ${mappedCustomRole || user.role}`
            );

            return res.status(403).json({
              status: false,
              message: `CRM login window is closed. You can login again at ${nextStartTime} IST.`,
          ...
          // ✅ Login is allowed, attach timing info to response
          const remainingMinutes = getRemainingMinutes(loginTiming.endTime);
          console.log(
            `✅ Login allowed for ${mappedCustomRole || user.role} - Remaining time: ${remainingMinutes} minutes`
          );
          req.loginTimingInfo = {
      ...
      console.error('Login timing check error:', timingErr);
      // Continue with login if timing check fails (non-blocking)
    }
    ...
    // If login timings are active and this role is restricted, token should expire at end time
    if (req.loginTimingInfo?.endTime) {
      try {
        const [endH, endM] = String(req.loginTimingInfo.endTime).split(':').map(Number);
        const now = new Date();
        const istNow = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
        const currentMinutes = istNow.getHours() * 60 + istNow.getMinutes();
        const endMinutes = endH * 60 + endM;

        const remainingMinutes = Math.max(1, endMinutes - currentMinutes);
        tokenExpiresIn = `${remainingMinutes}m`;
      } catch (tokenExpErr) {
        console.error('Failed to compute token expiry from login timing:', tokenExpErr);
      }
    }
    ...
    // Include login timing info in response if available
    const response = {
      status: true,
    ...
    if (req.loginTimingInfo) {
      response.loginTimingInfo = req.loginTimingInfo;
    }
...
exports.logoutUser = async (req, res) => {
  try {
    ...
    if (user.login_time && user.organizationId) {
      ...
      // Use IST start of day (IST midnight stored as UTC representation — do NOT use setHours)
      const startOfDay = moment.tz(loginTime, 'Asia/Kolkata').startOf('day').toDate();
...
exports.getCurrentActiveTime = async (req, res) => {
  try {
    ...
    // Management role should not have active time tracked
    ...
    return res.status(200).json({
      status: true,
      data: {
        activeTimeMinutes: parseFloat(currentActiveTime.toFixed(2)), // Return float (e.g. 0.55 mins)
        workStatus: user.workStatus,
      },
    });
  } catch (error) {
...
exports.getAllEmployees = async (req, res) => {
  try {
    ...
    const employees = await User.find({
      ...
      .select(
        'name email role customRole alias department address tier profileImage visiblePassword employee_id mobile is_active workStatus isBlocked login_time logout_time break_time breakLogs accumulatedActiveTime lastStatusChangeTime salary'
      )
      .sort({ createdAt: -1 });
    ...
    const employeesWithActiveTime = employees.map((emp) => {
      ...
      return {
        ...empObj,
        activeTime: parseFloat(currentActiveTime.toFixed(2)), // Return in minutes
      };
    });
...
exports.updateEmployeeStatus = async (req, res) => {
  try {
    ...
    // Logic: Only update isBlocked status as requested
    employee.isBlocked = isBlocked;

    if (isBlocked) {
      // 🛑 BLOCKING USER
      employee.blockedAt = now;
      employee.is_active = false
      employee.blockedReason = 'Account deactivated by Admin';
      employee.workStatus = 'offline';
    } else {
      // ✅ UNBLOCKING USER
      employee.blockedAt = null;
      employee.blockedReason = null;
      employee.failedLoginAttempts = 0; // Reset login attempts
    ...
    return res.status(200).json({
      message: isBlocked ? 'Employee blocked successfully' : 'Employee unblocked successfully',
      status: true,
      data: employee,
    });
  } catch (error) {
...
exports.updateEmployeeByAdmin = async (req, res) => {
  try {
    ...
    // Tier editable only for Agent/QA/TL/Dev; if role changed away from those, clear tier
    if (tier !== undefined) {
      if (['Agent', 'QA', 'TL', 'Dev'].includes(employee.role)) {
    ...
    // Handle alias field for Agent/QA/TL/Dev roles
    if (alias !== undefined) {
      if (['Agent', 'QA', 'TL', 'Dev'].includes(employee.role)) {
...
exports.getEscalationHierarchy = async (req, res) => {
  try {
    ...
    for (const emp of employees) {
      ...
      if (emp.tier) {
        // Agent, QA, TL — place in their assigned tier
        if (!hierarchy[emp.tier]) hierarchy[emp.tier] = {};
        if (!hierarchy[emp.tier][d]) hierarchy[emp.tier][d] = [];
        hierarchy[emp.tier][d].push(userData);
      } else {
        // Dev (and any tier-less role) — show in every tier so they
        // are visible for escalation regardless of the caller's tier
        tiers.forEach((t) => {
          if (!hierarchy[t]) hierarchy[t] = {};
          if (!hierarchy[t][d]) hierarchy[t][d] = [];
          hierarchy[t][d].push(userData);
        });
      }
    }
...
exports.get30DayActivity = async (req, res) => {
  try {
    ...
    // Check permissions: Admin/TL can view anyone, others can only view themselves
    ...
    // Get last 30 days of activity (IST-aware)
    ...
    const formattedActivities = activities.map((activity) => ({
      ...activity.toObject(),
      totalOnlineTime: parseFloat((activity.totalOnlineTime || 0).toFixed(2)),
      totalBreakTime: parseFloat((activity.totalBreakTime || 0).toFixed(2)),
    }));
...
exports.getActivityReport = async (req, res) => {
  try {
    const userId = req.user?._id || req.user?.id;
    const { period = 'daily', month, year, role } = req.query;
    ...
    if (month && year) {
      ...
      // IST start of month
      startDate = moment
        .tz({ year: yearNum, month: monthNum, day: 1 }, 'Asia/Kolkata')
        .startOf('day')
        .toDate();
      // IST end of month
      ...
      endDate = now.toDate(); // or endOf('day')? keeping previous logic which was 'now'
    } else {
      // Daily - today only (IST start of day)
      startDate = now.clone().startOf('day').toDate();
      endDate = now.toDate();
    }

    // Determine which role(s) to query - support 'role' query param for QA/TL reports
    // Valid roles: 'Agent', 'QA', 'TL'. Default to 'Agent' for backward compatibility
    const validRoles = ['Agent', 'QA', 'TL'];
    ...
    const agents = await User.find({
      ...
      .select('_id name email employee_id profileImage isBlocked role')
      .lean();
      ...
      return {
        ...
        avgOnlineTime:
          agentActivity.daysWorked > 0
            ? parseFloat((agentActivity.totalOnlineTime / agentActivity.daysWorked).toFixed(2))
            : 0,
      };
    });
