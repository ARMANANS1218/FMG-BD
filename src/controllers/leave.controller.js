...
const calculateDays = (startDate, endDate) => {
  ...
  const diffDays = end.diff(start, 'days') + 1; // +1 to include both start and end date
  return diffDays;
};
...
exports.applyLeave = async (req, res) => {
  try {
    ...
    // Validate dates in IST
    const start = moment.tz(startDate, 'Asia/Kolkata').startOf('day').toDate();
    const end = moment.tz(endDate, 'Asia/Kolkata').startOf('day').toDate();
