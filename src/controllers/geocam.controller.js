...
exports.createLink = async (req, res) => {
  try {
    ...
    const preferredLink = isLocalRequest ? links.localhost : links.primary;
...
exports.submitCapturePublic = async (req, res) => {
  try {
    ...
    // Reject very large payloads early to avoid upstream upload failures.
    if (imageData.length > 14 * 1024 * 1024) {
      return res.status(413).json({ success: false, message: 'Image too large. Please retake and try again.' });
    }

    // Upload to Cloudinary from data URI
    let uploadResult;
    try {
      uploadResult = await uploadToCloudinary(imageData, 'geocam');
    } catch (uploadErr) {
      console.error('submitCapturePublic upload error:', uploadErr && uploadErr.message ? uploadErr.message : uploadErr);
      return res.status(502).json({ success: false, message: 'Image upload failed. Please retake photo and try again.' });
    }
